"""	No tutorial-04, nós vamos realizar transformações na matriz de modelo de exibição (modelview) para mover a área que será renderizada na tela. Em outras palavras, movimentaremos a câmera para renderizar uma área diferente.
	Logo no início da nossa biblioteca LUtil.py, nós definimos as coordenadas x e y da câmera, com "gCameraX" e "gCameraY". A função initGL() é basicamente a mesma do tutorial-03, apenas adicionamos a função glPushMatrix() para salvar a matriz de modelo de exibição na pilha. No contexto atual (OpenGL 2.1) nós temos a matriz de projeção e de modelo de exibição (GL_PROJECTION e GL_MODELVIEW), além de outras duas apresentadas na documentação. Cada uma dessas matrizes possui uma pilha associada a ela, você pode adicionar uma cópia da matriz atual à pilha, salvando-a para mais tarde.
