...
class LSpriteSheet(LTexture):
	...
	def generateDataBuffer(self, origin):
		...
		if(self.getTextureID != 0 and len(self.mClips) > 0):
			...
			for i in range(0,totalSprites):
				...
				if(origin == LSPRITE_ORIGIN_TOP_LEFT):
					...
					vTop = GLdouble(0.0)
					vBottom = GLdouble(self.mClips[i].h)
					vLeft = GLdouble(-self.mClips[i].w)
					vRight = GLdouble(0.0)
				elif(origin == LSPRITE_ORIGIN_BOTTOM_LEFT):
					vTop = GLdouble(-self.mClips[i].h)
					vBottom = GLdouble(0.0)
					vLeft = GLdouble(0.0)
					vRight = GLdouble(self.mClips[i].w)
					...
					vTop = GLdouble(-self.mClips[i].h / 2)
					vBottom = GLdouble(self.mClips[i].h / 2)
					vLeft = GLdouble(-self.mClips[i].w / 2)
					vRight = GLdouble(self.mClips[i].w / 2)

				#Superior esquerda
				vertexData[spriteIndices[0]].position.x = GLdouble(vLeft)
				vertexData[spriteIndices[0]].position.y = GLdouble(vTop)
				...
				#Superior direita
				vertexData[spriteIndices[1]].position.x = GLdouble(vRight)
				vertexData[spriteIndices[1]].position.y = GLdouble(vTop)
				...
				#Inferior direita
				vertexData[spriteIndices[2]].position.x = GLdouble(vRight)
				vertexData[spriteIndices[2]].position.y = GLdouble(vBottom)
				...
				#Inferior esquerda
				vertexData[spriteIndices[3]].position.x = GLdouble(vLeft)
				vertexData[spriteIndices[3]].position.y = GLdouble(vBottom)
