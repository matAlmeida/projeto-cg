...
class LFont(LSpriteSheet):
	...
	# def loadFreeType(self, path, pixelSize):
	# 	error = NULL;

	# 	# Pega as dimensões da celula
	# 	cellH = 0
	# 	cellW = 0
	# 	maxBearing = 0
	# 	minHang = 0

	# 	# Caracteristicas do Caractere
	# 	bitmap = []
	# 	metrics = []
	...
	def loadBitmap(self,imagem):
		...
		if(self.loadPixelsFromFile(imagem)):
			...
			for i in range(0,PIXEL_COUNT,4):
				...
				self.mPixels[i+ALPHA_BYTE] = self.mPixels[i+RED_BYTE]
				self.mPixels[i+RED_BYTE] = 0xFF
				self.mPixels[i+GREEN_BYTE] = 0xFF
				self.mPixels[i+BLUE_BYTE] = 0xFF
			'''
